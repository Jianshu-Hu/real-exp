...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay a local LeRobot episode through the ROS deployment joint target topics "
            "and record a structured trace for offline analysis."
        )
    )
    ...
    parser.add_argument("--output", required=True, type=Path, help="Directory where trace/report inputs are written.")
    ...
    parser.add_argument("--left-target-topic", default="/left/deployment/joint_states")
    parser.add_argument("--right-target-topic", default="/right/deployment/joint_states")
...
def load_episode_data(dataset_root: Path, episode_index: int) -> EpisodeData:
    ...
    if arm_action_representation != "absolute_joint_position":
        raise ValueError(
            "Controller-matched replay currently requires arm_action_representation="
...
def binary_gripper_targets(values: np.ndarray, mode: str) -> np.ndarray:
    ...
    if max_value - min_value < 1e-6:
        return np.ones_like(values) if max_value > 0.01 else np.zeros_like(values)
    threshold = (min_value + max_value) * 0.5
    return np.where(values >= threshold, 1.0, 0.0)
...
def print_dry_run_summary(data: EpisodeData, mode: str, fps: float, no_gripper: bool) -> None:
    ...
    if not no_gripper:
        left = binary_gripper_targets(targets["left_gripper_raw"], mode)
        right = binary_gripper_targets(targets["right_gripper_raw"], mode)
...
def import_ros_dependencies() -> tuple[Any, Any, Any, Any]:
    try:
        ...
        raise RuntimeError(
            "ROS 2 Python dependencies are required for controller-matched replay. "
...
def build_replay_node_class(Node: Any, JointState: Any, Float32: Any) -> type:
    class ControllerMatchedReplayNode(Node):  # type: ignore[misc, valid-type]
        def __init__(self, args: argparse.Namespace) -> None:
            super().__init__("controller_matched_replay")
    ...
    return ControllerMatchedReplayNode
...
def run_replay(args: argparse.Namespace, data: EpisodeData, fps: float) -> None:
    ...
    left_gripper_targets = None if args.no_gripper else binary_gripper_targets(targets["left_gripper_raw"], args.mode)
    right_gripper_targets = None if args.no_gripper else binary_gripper_targets(targets["right_gripper_raw"], args.mode)
    ...
    try:
        ...
        print("Starting controller-matched replay. Type `q` + Enter to abort.")
